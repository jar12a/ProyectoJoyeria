...
<body>
    ...
    <div class="banner1 mb-4 position-relative">
        <img src="/ProyectoJoyeria/imagenes/103.jpg" class="img-fluid w-100" alt="Banner Cadenas">
        <div class="banner-description position-absolute text-white p-3 d-flex flex-column align-items-center justify-content-center">
            <h3>Cadenas Exclusivas</h3>
            <p>Descubre nuestra colección de cadenas elegantes y sofisticadas, perfectas para cualquier ocasión.</p>
        </div>
    </div>
    <div class="container mt-5">
        <!-- Sección de Productos -->
        <h2 class="mb-4">CADENAS</h2>
        <div class="row">
            <?php
            // Realizar la consulta a la base de datos para obtener solo los productos de la categoría "cadenas"
            ...
            while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                ...
                echo '                    <button class="btn btn-primary" type="button" id="btn-carrito2-' . $row['ID_Producto'] . '" onclick="agregarAlCarrito(' . $row['ID_Producto'] . ', \'' . $row['Nombre'] . '\', ' . $row['Precio'] . ', \'' . $row['Imagen'] . '\')">';
                ...
                echo '                            <div class="input-group-append">';
                echo '                                <button class="btn btn-primary" type="button" id="btn-modal-carrito2-' . $row['ID_Producto'] . '" onclick="agregarAlCarrito(' . $row['ID_Producto'] . ', \'' . $row['Nombre'] . '\', ' . $row['Precio'] . ', \'' . $row['Imagen'] . '\', true)">';
                echo '                                    <i class="bi bi-cart"></i> Agregar al carrito';
                echo '                                </button>';
                echo '                            </div>';
                echo '                        </div>';
    ...
    <script>
        ...
        function agregarAlCarrito(id, nombre, precio, imagen, isModal = false) {
            ...
            let boton = isModal ? document.getElementById(`btn-modal-carrito2-${id}`) : document.getElementById(`btn-carrito2-${id}`);
